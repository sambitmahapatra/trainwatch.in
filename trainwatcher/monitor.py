...
from . import logger
from . import best_model as best_model_module
from . import summary as summary_module
from . import config as config_module
from .exceptions import MonitorError, NotificationError, TrainWatcherError
...
def start() -> Dict[str, Any]:
    ...
    if _state.status == "running":
        raise MonitorError("TrainWatcher monitor already running.")
...
def set_best_model(candidate: Any) -> Dict[str, Any]:
    """Capture a duck-typed best-model summary for the current run."""
    summary = best_model_module.extract(candidate)
    if summary is None:
        raise MonitorError("Could not extract best-model information from candidate.")
    _state.best_model = summary
    return dict(summary)
...
def heartbeat(
    ...
    if _state.status != "running":
        raise MonitorError("TrainWatcher monitor is not running.")
...
def end(config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    resolved_config = _resolve_config(config)
    payload = summary_module.build_payload(snapshot(), logger.get_metrics(), config=resolved_config)
    summary_text = summary_module.render_payload(payload)
    _state.summary = summary_text
    _state.run_log_path = _maybe_write_run_log(resolved_config, payload)
...
def _warn_if_notifications_disabled() -> None:
    ...
    if not channels and not cloud_enabled:
        warnings.warn(
            "TrainWatcher: no notification channels enabled and no cloud credentials found; "
...
def _maybe_write_run_log(config: Dict[str, Any], payload: Dict[str, Any]) -> Optional[str]:
    ...
    payload["summary"] = _state.summary
    ...
    try:
        ...
        raise TrainWatcherError(f"Failed to write run log to {path}") from exc
